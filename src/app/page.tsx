...
export default function LandingPage() {
  ...
  return (
    <div className="relative flex-1 flex flex-col justify-between min-h-screen">
      {/* Glow overlays */}
      <div className="absolute top-20 left-1/4 w-[450px] h-[450px] bg-indigo-500/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-20 right-1/4 w-[450px] h-[450px] bg-purple-500/10 rounded-full blur-3xl pointer-events-none" />

      {/* Header */}
      <header className="w-full max-w-7xl mx-auto px-6 py-6 flex items-center justify-between z-10">
        <div className="flex items-center space-x-2">
          <div className="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-2.5 rounded-lg glow-indigo">
            <Sparkles className="h-5 w-5 text-white" />
          </div>
          <span className="font-bold text-xl tracking-tight text-white">ResearchGap <span className="gradient-text">AI</span></span>
        </div>
        <div>
          {session ? (
            <Link href="/dashboard">
            ...
            <div className="flex space-x-3">
              ...
              <Link href="/register">
                <Button className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white hover:opacity-90 shadow-lg shadow-indigo-500/20">Get Started</Button>
              </Link>
            </div>
          )}
        </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 flex flex-col items-center justify-center text-center py-16 z-10">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-indigo-500/20 bg-indigo-500/5 text-indigo-300 text-xs font-semibold uppercase tracking-wider mb-6 backdrop-blur">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-20">
          <Link href={session ? "/dashboard" : "/register"}>
            <Button size="lg" className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white hover:opacity-90 font-semibold px-8 shadow-lg shadow-indigo-500/20 flex items-center gap-2 hover:scale-[1.03] transition-all duration-200">
        ...
        <section id="features" className="w-full py-12 scroll-mt-24">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
            {features.map((feat, index) => (
              <div key={index} className="glass-card p-6 rounded-xl stagger-item border-slate-800">
                <div className="mb-4 bg-slate-900 p-3 rounded-lg inline-block border border-slate-800">
                  {feat.icon}
                </div>
        ...
        <section className="w-full mt-16 border-t border-slate-900 pt-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            <div className="flex flex-col items-center">
              ...
              <span className="text-slate-500 text-xs font-semibold uppercase tracking-wider">Literature Review Speed</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-4xl font-extrabold text-white mb-2 flex items-center gap-1">
                <CheckCircle className="h-6 w-6 text-purple-400" /> 95%
              </span>
              <span className="text-slate-500 text-xs font-semibold uppercase tracking-wider">Accuracy in Gap Identification</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-4xl font-extrabold text-white mb-2 flex items-center gap-1">
                <Network className="h-6 w-6 text-pink-400" /> 100%
              </span>
              <span className="text-slate-500 text-xs font-semibold uppercase tracking-wider">Scientific Rigor Mapping</span>
            </div>
          </div>
        </section>
      </main>
      ...
      {/* Footer */}
      <footer className="w-full border-t border-slate-900 py-8 text-center text-slate-500 text-sm z-10 bg-slate-950/40 backdrop-blur">
